...
import { 
  DrawArcText, CanvasMask, 
  CanvasTexture, CanvasResizeText, 
...
const FrontImageShirt = ({ color, slide, active, setMouseEnter, setActive, setMobileActive }) => {
  const { TextArray, setTextArray, setTextInput, initialObject } = useContext(StatesContext);
    const ArcCanvasRef = useRef(null);
    const targetRef = useRef(null);

    const [FrontShirtArray, setFrontShirtArray] = useState([]);
    ...
    const capturePreviewImage = () => {
      html2canvas(shirtRef.current).then(canvas => {
        const imgData = canvas.toDataURL('image/png');
        setPreviewImage(imgData);
      }).catch(err => {
        console.error("Error capturing preview image:", err);
      });
    };
    ...
    useEffect(() => {
      ...
      if (TextArray.length > 0) {
        const FrontShirtObjects = TextArray.filter(obj => obj.ShirtType === 'front');
        setFrontShirtArray(FrontShirtObjects);
      } else {
        setFrontShirtArray([]);
      }
    }, [TextArray]);
  ...
  return (
    <>
     <button onClick={capturePreviewImage} className="">Capture</button>
      {previewImage && (
        <div>
          <img src={previewImage} alt="Preview" />
        </div>
      )}
    <div ref={shirtRef} className={color.zoom ? " scale-125 overflow-hidden flex items-center justify-center h-full relative" : "relative flex items-center justify-center h-full"}>
      <CanvasMask color={color} />
      <CanvasTexture />
      <CanvasTexture />
      ...
      {slide ? (
        <div className="absolute inset-0 flex items-center justify-center z-[9999]">
          <div className="shirt-design relative">
            {FrontShirtArray?.map((textObj, index) => {
              return(
                <div
                  id="textObj"
                  key={index}
      ...
      ) : ((active === "1" || active === "2" || active === "3" || active === "4") && FrontShirtArray?.length > 0) ? (
        <div className="absolute inset-0 flex items-center justify-center z-[9999]">
          <div className="shirt-design relative" >
            {TextArray?.map((textObj, index) => {
             if( textObj?.ShirtType === 'front') {
              return(
                  <div
                    id="textObj"
                    key={index}
                      ...
                      >
                        ...
                        <div className="print-object-controls" 
                          ...
                          >
                          {
                            ...
                            {textObj?.clipArt?.path && (
                              <div
                                className={`cursor-move w-[${textObj?.imageSize ? `${textObj?.imageSize}px` : "100px"}] h-[${textObj?.imageSize ? `${textObj?.imageSize}px` : "100px"}]`}
                                onClick={() =>{
                            ...
                            {textObj?.uploadImage?.path && (
                                ...
                                >
                                  <img
                                    // className={`w-[${textObj?.UploadSize}px] h-[${textObj?.UploadSize}px]`}
                                    src={textObj?.uploadImage?.path}   
                                    style={{filter: textObj?.uploadImage?.filter ,width : `${textObj?.UploadSize}px`, height: `${textObj?.UploadSize}px` }}                           
                                  />
                                </div>
                            )}
      ...
      ) : (TextArray?.length === 0 || FrontImageShirt?.length === 0) && (
